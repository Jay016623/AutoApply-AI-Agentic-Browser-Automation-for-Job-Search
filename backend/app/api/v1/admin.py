...
from app.services.queue import get_queue_depth, queue_diagnostics
...
async def diagnostics(
    ...
    return OpsDiagnosticsResponse(
        queue_depths=(
            await queue_diagnostics(redis, QUEUE_APPLY, QUEUE_APPLY_DEAD_LETTER)
            if redis is not None
            else {"queue_size": 0, "inflight_jobs": 0, "dead_letter_count": 0}
        ),
